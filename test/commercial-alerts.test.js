...
const { alertConfig, alertEventFor, sendCommercialAlert } = require('../commercial-alerts');
...
test('uses an isolated alert recipient and approved content SID', async () => {
  ...
  const result=await sendCommercialAlert({event:'VALUATION_REQUESTED',patient:'TEST',treatment:'Diseño de Sonrisa'}, {env,client});
